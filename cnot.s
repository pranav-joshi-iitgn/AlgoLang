...
# Definition : x is -36($s0)

# Definition : i is -40($s0)
...
add $t1,$zero,$zero
# Measurement
measure $t2 #this stores the measurement in the 0th index of the register $t1
# Storing measurement into x
# getting frame containing x
add $t0,$s0,$zero
# setting x
# assuming value in $t1
sw $t1,-36($t0)
# skipping pop

# getting frame containing count
add $t0,$s0,$zero
# getting count
addi $s1,$s1,-4
lw $t1,-44($t0)
...
label_t1_15:
slt $t7,$zero,$t3 # t3 > 0
beq $t7,$zero,label_t2_15 # done adding
addi $t6,$t6,4
addi $t1,$t1,4
lw $t7,0($t1)
sw $t7,0($t6)
addi $t3,$t3,-4
j label_t1_15
label_t2_15: # add struff from second list
slt $t7,$zero,$t4 # t3 > 0
beq $t7,$zero,label_t2_end_15 # done adding
addi $t6,$t6,4
addi $t2,$t2,4
lw $t7,0($t2)
sw $t7,0($t6)
addi $t4,$t4,-4
j label_t2_15
label_t2_end_15: # end adding from 2nd list
add $t1,$t0,$zero
...
j label21 # skip conversion
label20: # f1 is int
cvt.s.w $f1,$f1
label21: # f1 is finally a float
...
j label23 # skip conversion
label22: # f2 is int
cvt.s.w $f2,$f2
label23: # f2 is finally a float
