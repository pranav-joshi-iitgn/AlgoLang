...
addi $t3,$zero,7
addi $t4,$zero,4
beq $t1,$zero,label9 # 000 -> int
beq $t1,$t3,label9 # 111 -> int
beq $t1,$t4,label_alg9 # 100 -> alg .. printed as int
addi $t3,$zero,3
bne $t1,$t3,label8 # 011 is for str

# print a string
lw $t1,0($t0) # 4n
addi $v0,$zero,11 # for printing characters
label7: # print character routine
slt $t3,$zero,$t1
beq $t3,$zero,label10 # if t1 <= 0, finish
addi $t0,$t0,4 # next character
lw $a0,0($t0) #put char in buffer
syscall # print char
addi $t1,$t1,-4 # decr remaining bytes by 1
j label7 # continue printing characters
...
beq $t1,$t4,label_alg14 # 100 -> alg .. printed as int
addi $t3,$zero,3
bne $t1,$t3,label13 # 011 is for str

# print a string
lw $t1,0($t0) # 4n
addi $v0,$zero,11 # for printing characters
label12: # print character routine
slt $t3,$zero,$t1
...
syscall # print char
addi $t1,$t1,-4 # decr remaining bytes by 1
j label12 # continue printing characters
...
bne $t9,$zero,label21 # el..
lw $t9,0($s1)
addi $s1,$s1,4
beq $t9,$zero,label21 # ..if
...
addi $t3,$zero,7
addi $t4,$zero,4
beq $t1,$zero,label24 # 000 -> int
beq $t1,$t3,label24 # 111 -> int
beq $t1,$t4,label_alg24 # 100 -> alg .. printed as int
addi $t3,$zero,3
bne $t1,$t3,label23 # 011 is for str

# print a string
lw $t1,0($t0) # 4n
addi $v0,$zero,11 # for printing characters
label22: # print character routine
slt $t3,$zero,$t1
beq $t3,$zero,label25 # if t1 <= 0, finish
addi $t0,$t0,4 # next character
lw $a0,0($t0) #put char in buffer
syscall # print char
addi $t1,$t1,-4 # decr remaining bytes by 1
j label22 # continue printing characters
...
addi $t3,$zero,7
addi $t4,$zero,4
beq $t1,$zero,label30 # 000 -> int
beq $t1,$t3,label30 # 111 -> int
beq $t1,$t4,label_alg30 # 100 -> alg .. printed as int
addi $t3,$zero,3
bne $t1,$t3,label29 # 011 is for str

# print a string
lw $t1,0($t0) # 4n
addi $v0,$zero,11 # for printing characters
label28: # print character routine
slt $t3,$zero,$t1
beq $t3,$zero,label31 # if t1 <= 0, finish
addi $t0,$t0,4 # next character
lw $a0,0($t0) #put char in buffer
syscall # print char
addi $t1,$t1,-4 # decr remaining bytes by 1
j label28 # continue printing characters
