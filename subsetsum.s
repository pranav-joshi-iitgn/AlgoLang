...
cleanup_c2s_stack_v3:
    # Restore initial $s5 (was saved first, before any register saving frames)
    lw   $s5, 0($s1)       # $s1 now points to where initial $s5 was saved
...
addi $s5,$s5,60 # make space for 16 elements on heap
# store element at index 14
lw $t1,0($s1)
sw $t1,-4($s5)
# store element at index 13
lw $t1,4($s1)
sw $t1,-8($s5)
# store element at index 12
lw $t1,8($s1)
sw $t1,-12($s5)
# store element at index 11
lw $t1,12($s1)
sw $t1,-16($s5)
# store element at index 10
lw $t1,16($s1)
sw $t1,-20($s5)
# store element at index 9
lw $t1,20($s1)
sw $t1,-24($s5)
# store element at index 8
lw $t1,24($s1)
sw $t1,-28($s5)
# store element at index 7
lw $t1,28($s1)
sw $t1,-32($s5)
# store element at index 6
lw $t1,32($s1)
sw $t1,-36($s5)
# store element at index 5
lw $t1,36($s1)
sw $t1,-40($s5)
# store element at index 4
lw $t1,40($s1)
sw $t1,-44($s5)
# store element at index 3
lw $t1,44($s1)
sw $t1,-48($s5)
...
lw $t1,56($s1)
sw $t1,-60($s5)
addi $s1,$s1,56 # pop stack 14 times
addi $t0,$s5,-64 # old $s5 value
...
add $t4,$t8,$zero # t4 = 1
label12_loop:
beq $t2,$zero,label12_out
and $t3,$t2,$t8
beq $t3,$zero,label12_in
mult $t4,$t1
mflo $t4 # t4 = t4*t1
label12_in: # mult done
mult $t1,$t1
mflo $t1 # square
srl $t2,$t2,1
j label12_loop
...
lw $t1,0($s1) # get value
addi $t0,$s0,-52 # load variable address
sw $t1,0($t0) # update the value at variable address
addi $s1,$s1,4 # remove the value on stack

# Definition : j is -56($s0)

# Definition : x is -60($s0)

# Definition : S is -64($s0)
...
sll $t2,$t2,2 # t2 = t2*4
lw $t0,-36($s0) # load pointer value
...
mtc1 $t1,$f1
mtc1 $t2,$f2
bne $t7,$zero,label_float16 # if t1 was a float too, jump to adition
cvt.s.w $f1,$f1 # if not, convert t1 to float
j label_float16 # j to float addition

label16: # t2 is int
bne $t7,$zero,label_float_conv16 # if t1 was a float, jump to conversion

label_int16: # int addition
add $t1,$t1,$t2
j label18 # finish

label_float_conv16: # conversion of t2 to float
mtc1 $t1,$f1
mtc1 $t2,$f2
cvt.s.w $f2,$f2

label_float16: # float addition
add.s $f1,$f1,$f2
mfc1 $t1,$f1
j label18 #finish
...
label_t1_18:
slt $t7,$zero,$t3 # t3 > 0
beq $t7,$zero,label_t2_18 # done adding
addi $t6,$t6,4
addi $t1,$t1,4
lw $t7,0($t1)
sw $t7,0($t6)
addi $t3,$t3,-4
j label_t1_18
label_t2_18: # add struff from second list
slt $t7,$zero,$t4 # t3 > 0
beq $t7,$zero,label_t2_end_18 # done adding
addi $t6,$t6,4
addi $t2,$t2,4
lw $t7,0($t2)
sw $t7,0($t6)
addi $t4,$t4,-4
j label_t2_18
label_t2_end_18: # end adding from 2nd list
add $t1,$t0,$zero
...
mtc1 $t1,$f1
mtc1 $t2,$f2
bne $t7,$zero,label_float21 # if t1 was a float too, jump to adition
cvt.s.w $f1,$f1 # if not, convert t1 to float
j label_float21 # j to float addition

label21: # t2 is int
bne $t7,$zero,label_float_conv21 # if t1 was a float, jump to conversion

label_int21: # int addition
add $t1,$t1,$t2
j label23 # finish

label_float_conv21: # conversion of t2 to float
mtc1 $t1,$f1
mtc1 $t2,$f2
cvt.s.w $f2,$f2

label_float21: # float addition
add.s $f1,$f1,$f2
mfc1 $t1,$f1
j label23 #finish
...
label_t1_23:
slt $t7,$zero,$t3 # t3 > 0
beq $t7,$zero,label_t2_23 # done adding
addi $t6,$t6,4
addi $t1,$t1,4
lw $t7,0($t1)
sw $t7,0($t6)
addi $t3,$t3,-4
j label_t1_23
label_t2_23: # add struff from second list
slt $t7,$zero,$t4 # t3 > 0
beq $t7,$zero,label_t2_end_23 # done adding
addi $t6,$t6,4
addi $t2,$t2,4
lw $t7,0($t2)
sw $t7,0($t6)
addi $t4,$t4,-4
j label_t2_23
label_t2_end_23: # end adding from 2nd list
add $t1,$t0,$zero
...
lw $t9,0($s1) #get result of condition
addi $s1,$s1,4 # delete a value
beq $t9,$zero,label33 # if

# putting "found at permutation :" on heap 
add $t0,$s5,$zero
addi $s5, $s5,92
addi $t1,$zero,88 # add size at start
sw $t1,0($t0)
addi $t1,$zero,102 # f
sw $t1, 4($t0)
addi $t1,$zero,111 # o
...
mtc1 $t1,$f1
mtc1 $t2,$f2
bne $t7,$zero,label_float26 # if t1 was a float too, jump to adition
cvt.s.w $f1,$f1 # if not, convert t1 to float
j label_float26 # j to float addition

label26: # t2 is int
bne $t7,$zero,label_float_conv26 # if t1 was a float, jump to conversion

label_int26: # int addition
add $t1,$t1,$t2
j label28 # finish

label_float_conv26: # conversion of t2 to float
mtc1 $t1,$f1
mtc1 $t2,$f2
cvt.s.w $f2,$f2

label_float26: # float addition
add.s $f1,$f1,$f2
mfc1 $t1,$f1
j label28 #finish
...
label_t1_28:
slt $t7,$zero,$t3 # t3 > 0
beq $t7,$zero,label_t2_28 # done adding
addi $t6,$t6,4
addi $t1,$t1,4
lw $t7,0($t1)
sw $t7,0($t6)
addi $t3,$t3,-4
j label_t1_28
label_t2_28: # add struff from second list
slt $t7,$zero,$t4 # t3 > 0
beq $t7,$zero,label_t2_end_28 # done adding
addi $t6,$t6,4
addi $t2,$t2,4
lw $t7,0($t2)
sw $t7,0($t6)
addi $t4,$t4,-4
j label_t2_28
label_t2_end_28: # end adding from 2nd list
add $t1,$t0,$zero
...
mtc1 $t1,$f1
mtc1 $t2,$f2
bne $t7,$zero,label_float35 # if t1 was a float too, jump to adition
cvt.s.w $f1,$f1 # if not, convert t1 to float
j label_float35 # j to float addition

label35: # t2 is int
bne $t7,$zero,label_float_conv35 # if t1 was a float, jump to conversion

label_int35: # int addition
add $t1,$t1,$t2
j label37 # finish

label_float_conv35: # conversion of t2 to float
mtc1 $t1,$f1
mtc1 $t2,$f2
cvt.s.w $f2,$f2

label_float35: # float addition
add.s $f1,$f1,$f2
mfc1 $t1,$f1
j label37 #finish
...
label_t1_37:
slt $t7,$zero,$t3 # t3 > 0
beq $t7,$zero,label_t2_37 # done adding
addi $t6,$t6,4
addi $t1,$t1,4
lw $t7,0($t1)
sw $t7,0($t6)
addi $t3,$t3,-4
j label_t1_37
label_t2_37: # add struff from second list
slt $t7,$zero,$t4 # t3 > 0
beq $t7,$zero,label_t2_end_37 # done adding
addi $t6,$t6,4
addi $t2,$t2,4
lw $t7,0($t2)
sw $t7,0($t6)
addi $t4,$t4,-4
j label_t2_37
label_t2_end_37: # end adding from 2nd list
add $t1,$t0,$zero
...
# putting "not found" on heap 
add $t0,$s5,$zero
addi $s5, $s5,40
addi $t1,$zero,36 # add size at start
sw $t1,0($t0)
addi $t1,$zero,110 # n
sw $t1, 4($t0)
addi $t1,$zero,111 # o
...
addi $t3,$zero,7
addi $t4,$zero,4
beq $t1,$zero,label40 # 000 -> int
beq $t1,$t3,label40 # 111 -> int
beq $t1,$t4,label_alg40 # 100 -> alg .. printed as int
addi $t3,$zero,3
bne $t1,$t3,label39 # 011 is for str

# print a string
lw $t1,0($t0) # 4n
addi $v0,$zero,11 # for printing characters
label38: # print character routine
slt $t3,$zero,$t1
beq $t3,$zero,label41 # if t1 <= 0, finish
addi $t0,$t0,4 # next character
lw $a0,0($t0) #put char in buffer
syscall # print char
addi $t1,$t1,-4 # decr remaining bytes by 1
j label38 # continue printing characters
